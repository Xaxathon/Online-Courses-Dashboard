...
interface ProtocolAddUserModalProps {
	onClose: () => void;
...
const MIN_TASKS_TO_SHOW_MESSAGE = 2;
const SEARCH_DELAY = 300;

const ProtocolAddUserModal: React.FC<ProtocolAddUserModalProps> = ({
	onClose,
	onUserSelect,
}) => {
	...
	const toggleCreating = () => {
		...
		if (isCreating) {
			// Reset form state when closing the create form
			setServerError(null);
		}
	};
	...
	const showNoMoreUsersMessage =
		!isFetching &&
		initialLoadComplete &&
		allUsers.length >= MIN_TASKS_TO_SHOW_MESSAGE &&
		!isSearchMode &&
...
export default ProtocolAddUserModal;
